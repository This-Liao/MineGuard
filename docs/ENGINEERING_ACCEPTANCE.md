...
验收日期：2026-09-13。本次在既有 LangChain4j、Lucene BM25、Milvus 与 RRF 链路上新增扩展检索集和独立扰动鲁棒性评测。2026-09-12 的 132 项测试 / 78.41% 仍保留其日期，不再作为当前数字展示。

## 干净构建结果

| 验收 | 命令 | 结果 |
| --- | --- | --- |
| 后端 | `mvn -q -Pexternal-it clean verify` | Surefire **136/136**，0 失败、0 异常、0 跳过 |
...
| JaCoCo 指令 | `verify` 阶段生成并执行 ≥ 70% 门禁 | **74.70%**，17,505 / 23,433；5,928 条未覆盖 |
...
摘要与分母见 [工程快照 JSON](eval/engineering-2026-09-13.json)。LangChain4j [真实调用报告](eval/langchain4j-smoke-2026-09-12.json) 与 v3 混合检索 [完整排名报告](eval/hybrid-retrieval-v3/report.json) 独立保存，不计入 JUnit 通过数。完整 XML 与 HTML 由 CI artifact 保存 14 天；本地 `target/` 会被下次 clean 清理。
