...
镜像拉取最多尝试 3 次，并按 15 秒、30 秒递增等待，用于处理容器仓库的瞬时网络错误。数据库启动、健康检查和 Maven 测试本身不自动重试，避免把稳定复现的工程缺陷误报为通过。

结束时仅清理该临时 runner 的 `mineguard-integration` Compose 项目和卷；不操作开发机上已运行的数据库。报告不包含本地账号文件、密钥文件或完整运行目录。

外部工作流始终归档 Compose 容器状态、镜像列表和服务日志；测试开始后还会归档测试节点日志与分布式结果摘要，便于区分镜像拉取、容器健康检查和故障注入问题。独立 JVM 根据操作系统选择当前 JDK 的 `java` / `java.exe`，显式使用离线哈希检索，不继承开发机的向量服务设置。
