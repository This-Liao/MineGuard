...
try {
    # v3 使用隔离的 Milvus 集合，并在一次运行中完成扩展主对照与独立扰动评测。
    ...
    mvn -q -DskipTests compile exec:java '-Dexec.mainClass=com.mineguard.eval.HybridRetrievalV3EvalApplication'
    if ($LASTEXITCODE -ne 0) { throw 'v3 混合检索评测未完成，请检查 data/runtime/hybrid-retrieval-v3。' }
} finally {
